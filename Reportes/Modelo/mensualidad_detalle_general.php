...
class Mensualidad_detalle
{
    ...
    public function listar_mensualidad_detalle_general()
    {
        $sql = "SELECT
                    ...
                    matricula_mes.id AS matricula_mes_id,
                    matricula_mes.nombre AS matricula_mes_nombre,
                    mensualidad_detalle.monto AS detalle_monto,
                    CASE WHEN mensualidad_detalle.pagado = 1 THEN 'PAGADO' WHEN mensualidad_detalle.pagado = 0 THEN 'PENDIENTE' ELSE 'DESCONOCIDO' END AS detalle_estado_pago,
                    mensualidad_detalle.observaciones AS detalle_observaciones,
                    CASE WHEN mensualidad_detalle.estado = 1 THEN 'ACTIVO' WHEN mensualidad_detalle.estado = 0 THEN 'INACTIVO' ELSE 'DESCONOCIDO' END AS detalle_estado
                FROM
                    mensualidad_detalle
                LEFT JOIN matricula_mes ON mensualidad_detalle.matricula_mes_id = matricula_mes.id
                ...
                WHERE
                    mensualidad_detalle.estado = 1 AND matricula_mes.estado = 1 AND matricula_detalle.estado = 1 AND usuario_apoderado.estado = 1
                    AND usuario_alumno.estado = 1 AND matricula.estado = 1 AND institucion_seccion.estado = 1 AND institucion_grado.estado = 1
                    AND institucion_nivel.estado = 1 AND institucion_lectivo.estado = 1
                ORDER BY
