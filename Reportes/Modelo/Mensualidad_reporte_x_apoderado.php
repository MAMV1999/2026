...
class Reportemensualidadxapoderado
{
    ...
    public function listar()
    {
        $sql = "SELECT 
                    ...
                    GROUP_CONCAT(md.recibo ORDER BY mm.id ASC SEPARATOR ', ') AS recibos,
                ...
                INNER JOIN matricula_detalle mdd 
                    ON md.id_matricula_detalle = mdd.id 
                    AND mdd.estado = 1

                INNER JOIN usuario_alumno ma 
                    ON mdd.id_usuario_alumno = ma.id 
                    AND ma.estado = 1

                INNER JOIN usuario_documento ud_alumno 
                    ON ma.id_documento = ud_alumno.id 
                    AND ud_alumno.estado = 1

                INNER JOIN usuario_apoderado ua 
                    ON mdd.id_usuario_apoderado = ua.id 
                    AND ua.estado = 1

                INNER JOIN usuario_apoderado_tipo uat 
                    ON ua.id_apoderado_tipo = uat.id 
                    AND uat.estado = 1

                INNER JOIN usuario_documento ud_apoderado 
                    ON ua.id_documento = ud_apoderado.id 
                    AND ud_apoderado.estado = 1

                INNER JOIN matricula m 
                    ON mdd.id_matricula = m.id 
                    AND m.estado = 1

                INNER JOIN institucion_seccion ise 
                    ON m.id_institucion_seccion = ise.id 
                    AND ise.estado = 1

                INNER JOIN institucion_grado ig 
                    ON ise.id_institucion_grado = ig.id 
                    AND ig.estado = 1

                INNER JOIN institucion_nivel iniv 
