...
class Reportemensualidadxapoderado
{
    ...
    public function listar()
    {
        $sql = "SELECT 
                ...
                LEFT JOIN matricula_mes mm 
                    ON md.matricula_mes_id = mm.id AND mm.estado = 1
                LEFT JOIN matricula_detalle mdd 
                    ON md.id_matricula_detalle = mdd.id AND mdd.estado = 1
                LEFT JOIN usuario_alumno ma 
                    ON mdd.id_usuario_alumno = ma.id AND ma.estado = 1
                LEFT JOIN usuario_documento ud_alumno 
                    ON ma.id_documento = ud_alumno.id AND ud_alumno.estado = 1
                LEFT JOIN usuario_apoderado ua 
                    ON mdd.id_usuario_apoderado = ua.id AND ua.estado = 1
                LEFT JOIN usuario_apoderado_tipo uat 
                    ON ua.id_apoderado_tipo = uat.id AND uat.estado = 1
                LEFT JOIN usuario_documento ud_apoderado 
                    ON ua.id_documento = ud_apoderado.id AND ud_apoderado.estado = 1
                LEFT JOIN matricula m 
                    ON mdd.id_matricula = m.id AND m.estado = 1
                LEFT JOIN institucion_seccion ise 
                    ON m.id_institucion_seccion = ise.id AND ise.estado = 1
                LEFT JOIN institucion_grado ig 
                    ON ise.id_institucion_grado = ig.id AND ig.estado = 1
                ...
                GROUP BY 
                    md.id_matricula_detalle,
                    i.nombre, i.telefono, i.correo, i.ruc, i.razon_social, i.direccion,
                    il.nombre, iniv.nombre, ig.nombre, ise.nombre,
                    ud_alumno.nombre, ma.numerodocumento, ma.nombreyapellido,
                    uat.nombre, ud_apoderado.nombre,
                    ua.numerodocumento, ua.nombreyapellido, ua.telefono
