...
class Facturacion_x_apoderado
{
    ...
    public function guardarEditarMasivo($detalles)
    {
        try {
            foreach ($detalles as $detalle) {
                ...
                $recibo = isset($detalle['recibo']) ? limpiarcadena($detalle['recibo']) : 0;
    ...
    public function listar()
    {
        $sql = "SELECT 
                ...
                INNER JOIN usuario_alumno ual ON mdeta.id_usuario_alumno = ual.id AND ual.estado = 1
    ...
    public function listar_frm($id)
    {
        ...
        $sql = "SELECT
                    ...
                    GROUP_CONCAT(CASE WHEN md.monto > 0 THEN COALESCE(md.observaciones, '') END ORDER BY mm.id ASC SEPARATOR ', ') AS observaciones
                ...
                JOIN matricula_mes mm ON md.matricula_mes_id = mm.id AND mm.institucion_lectivo_id = il.id
                ...
                AND il.estado = 1 AND mm.estado = 1 AND uat.estado = 1 AND ud.estado = 1
                AND md.pagado = 1 AND md.monto > 0
                
                GROUP BY
                    ua.id, ua.nombreyapellido, ua.numerodocumento, ua.telefono,
                    uat.nombre, ud.nombre, il.nombre, iniv.nombre, ig.nombre,
                    isec.nombre, ual.id, ual.numerodocumento, ual.nombreyapellido
