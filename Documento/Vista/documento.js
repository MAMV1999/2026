...
function cargar_responsables() {
    $.post(link + "listar_responsables_activos", function (r) {
        $("#id_documento_responsable").html(r);
    });
}
...
// Función para guardar o editar un documento
...
function mostrar(id) {
    $.post(link + "mostrar", { id: id }, function (data, status) {
        ...
        $("#id").val(data.id);
        $("#id_documento_responsable").val(data.id_documento_responsable);
        ...
        $("#id_documento_responsable").selectpicker("refresh");
    });
}
